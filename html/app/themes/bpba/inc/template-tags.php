...
if ( ! function_exists( 'bpba_2016_posted_on' ) ) :
	/**
	 * Prints HTML with meta information for the current post-date/time and author.
	 */
	function bpba_2016_posted_on() {
...
if ( ! function_exists( 'ba_entry_footer' ) ) :
	...
	function ba_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list && bpba_2016_categorized_blog() ) {
 ...
 * @return bool
 */
function bpba_2016_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'bpba_2016_categories' ) ) ) {
		...
		set_transient( 'bpba_2016_categories', $all_the_cool_cats );
	}

	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so bpba_2016_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so bpba_2016_categorized_blog should return false.
		return false;
	}
}

/**
 * Flush out the transients used in bpba_2016_categorized_blog.
 */
function bpba_2016_category_transient_flusher() {
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return;
	}
	// Like, beat it. Dig?
	delete_transient( 'bpba_2016_categories' );
}
add_action( 'edit_category', 'bpba_2016_category_transient_flusher' );
add_action( 'save_post',     'bpba_2016_category_transient_flusher' );
